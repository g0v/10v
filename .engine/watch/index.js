...
(function(){
  var fs, fsExtra, chokidar, path, debounce, i18n, PugTree, StylusTree, pug, stylus, lsc, aux, bundle, ret;
  fs = require('fs');
  ...
  ret = function(backend){
    var log, watch, process, this$ = this;
    ...
    watch = {
      ...
      init: function(opt){
        ...
        return i18n(opt.i18n).then(function(i18n){
          ...
          if ((build.watcher || (build.watcher = {})).ignores) {
            this$.ignores = (build.watcher || (build.watcher = {})).ignores;
          }
          ...
          this$.assets(build.assets) || [];
          return log.info("watching src for file change".cyan);
        });
      },
      custom: function(arg$){
      ...
      assets: function(assets){
        ...
        if (!fs.existsSync(modpath)) {
          return log.error("assets building: node_modules dir not found.".red);
        }
        desdir = function(f){
        ...
        add = function(src){
          ...
          return log.info("assets: " + src + " -> " + des + " ");
        };
        remove = function(src){
          var des;
          des = path.join(desdir(src), path.basename(src));
          ...
          fs.unlinkSync(des);
          return log.info(("assets: " + src + " -> " + des + " deleted.").yellow);
        };
      ...
      update: function(f){
        ...
        try {
          ...
          return log.error({
            err: err
          }, "Update failed with following information: ".red);
        }
      }
    };
